...
After the release is deployed, `saga tell <version>` dispatches each tale to its destinations. The wiring from audience to destination lives in `.saga/config.jsonnet` as **routes**: each audience declares one or more routes, and each route names a **courier** (like `slack` or `basecamp`) with the configuration that courier needs (a channel name, a project ID, and so on). For every tale in the chapter, saga looks up the matching audience's routes and invokes the courier once per tale, passing it a structured payload. The courier is a standalone binary on your PATH named `saga-courier-<name>` that turns that payload into a real message on its platform.
...
go install github.com/saga-changelog/saga/couriers/saga-courier-slack@latest
go install github.com/saga-changelog/saga/couriers/saga-courier-basecamp@latest
```
...
- [`saga-courier-basecamp`](./couriers/saga-courier-basecamp/README.md): posts tales to a Basecamp message board
- [`saga-courier-slack`](./couriers/saga-courier-slack/README.md): delivers tales to Slack channels via incoming webhooks
- [`saga-courier-stdout`](./couriers/saga-courier-stdout/README.md): prints tales to stdout, useful for testing and local development
